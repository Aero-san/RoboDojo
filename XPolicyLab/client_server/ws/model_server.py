    ...
    async def _handle_connection(self, websocket: ServerConnection) -> None:
        ...
        async def respond(frame: Frame) -> None:
            try:
                ...
                logger.debug("policy websocket closed while sending response")
            except Exception:
        ...
        try:
            ...
            # A client that disappears during the close handshake or a
            # keepalive timeout is a normal transport-level shutdown.
            logger.debug("policy websocket connection closed during teardown")
        finally:
            if pending:
...
def main() -> None:
    ...
    server = PolicyServer(
        ...
        PolicyServerConfig(
            ...
            ws_ping_interval_s=deploy_cfg.get("ws_ping_interval_s", 60.0),
            ws_ping_timeout_s=deploy_cfg.get("ws_ping_timeout_s", 120.0),
        ),
    )
