...
POSTTRAIN_OVERRIDES=()
if [[ -n "${POSTTRAIN_MODE:-}" ]]; then
    echo "[SERVER] Using post-training mode: ${POSTTRAIN_MODE}"
    POSTTRAIN_OVERRIDES+=(posttrain_mode="${POSTTRAIN_MODE}")
fi
if [[ -n "${POSTTRAIN_CHECKPOINT:-}" ]]; then
    echo "[SERVER] Using post-training artifact: ${POSTTRAIN_CHECKPOINT}"
    POSTTRAIN_OVERRIDES+=(posttrain_checkpoint="${POSTTRAIN_CHECKPOINT}")
fi
