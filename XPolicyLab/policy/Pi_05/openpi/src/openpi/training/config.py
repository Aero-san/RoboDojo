...
import openpi.models.pi0_fast as pi0_fast  # noqa: F401
...
import openpi.training.misc.polaris_config as polaris_config  # noqa: F401
import openpi.training.misc.roboarena_config as roboarena_config  # noqa: F401
...
FinetuneMode: TypeAlias = Literal[
    "full",
    "action_expert",
    "action_expert_lora",
    "paligemma_lora",
...
ShardingStrategy: TypeAlias = Literal["full_shard", "shard_grad_op", "no_shard"]
...
class LeRobotAlohaDataConfig(DataConfigFactory):
    ...
    # Physical action width returned by the policy. RoboDojo overrides this
    # from env_cfg for single-arm and end-effector-control checkpoints.
...
class TrainConfig:
    ...
    # Storage dtype for JAX/NNX model parameters and optimizer states. This is
    # separate from ``model.dtype``, which controls model compute.
    ...
    # Optional RoboDojo Pi0.5 parameter-selection policy. None preserves a
    # programmatically supplied freeze_filter; train.sh always selects a mode.
    ...
    # RECAP policy objective coefficient. None keeps the standard single-branch
    # flow-matching objective; a non-negative value enables
    # L_unconditional + recap_beta * L_conditioned.
    recap_beta: float | None = None

    # Used to pass metadata to the policy server.
    policy_metadata: dict[str, Any] | None = None

    # ``full_shard`` shards parameters, gradients and optimizer state for lower
    # per-GPU memory; ``shard_grad_op`` keeps parameters replicated and is
    # generally faster when the model fits; ``no_shard`` replicates the full
    # state. Default to the memory-safe behavior used by the original trainer.
    sharding_strategy: ShardingStrategy = "full_shard"
    # Keep optimizer state in host memory between steps. This saves GPU memory
    # but adds host-device transfers and is therefore disabled by default.
    cpu_offload: bool = False
    # If the value is greater than 1, FSDP is enabled across this many devices;
    # the remaining mesh dimension runs data parallel groups.
    fsdp_devices: int = 1
    ...
    def __post_init__(self) -> None:
        ...
        if self.parameter_dtype not in {"bfloat16", "float32"}:
            raise ValueError(
                f"parameter_dtype must be 'bfloat16' or 'float32', got {self.parameter_dtype!r}."
            )
...
def _as_lora_variant(variant: str) -> str:
    if variant.endswith("_lora"):
        return variant
    lora_variant = f"{variant}_lora"
    if lora_variant not in {"gemma_300m_lora", "gemma_2b_lora"}:
        raise ValueError(f"No LoRA variant is available for {variant!r}.")
    return lora_variant


def resolve_finetune_config(config: TrainConfig) -> TrainConfig:
    """Resolve a Pi0.5 fine-tuning mode into model variants and a freeze filter."""
    mode = config.finetune_mode
    if mode is None:
        return config
    if not isinstance(config.model, pi0_config.Pi0Config) or not config.model.pi05:
        raise ValueError("finetune_mode is only supported for Pi0.5 Pi0Config models.")
    ...
    if mode == "full":
        ...
        if "lora" in model.paligemma_variant or "lora" in model.action_expert_variant:
            raise ValueError("action_expert mode requires non-LoRA model variants.")
        freeze_filter = nnx.Not(nnx_utils.PathRegex(action_expert_path))
    elif mode == "action_expert_lora":
        model = dataclasses.replace(
            model,
            action_expert_variant=_as_lora_variant(model.action_expert_variant),
        )
        freeze_filter = nnx.Not(nnx_utils.PathRegex(action_expert_lora_path))
    elif mode == "paligemma_lora":
        model = dataclasses.replace(
            model,
            paligemma_variant=_as_lora_variant(model.paligemma_variant),
        )
        ...
        raise ValueError(f"Unsupported Pi0.5 fine-tuning mode: {mode}")

    return dataclasses.replace(config, model=model, freeze_filter=freeze_filter)
