...
2. Training: this RoboDojo Pi0.5 adapter stores model parameters and AdamW
   states in bfloat16 by default. Use `--parameter-dtype=float32` (or
   `OPENPI_PARAMETER_DTYPE=float32` in `train.sh`) for full float32 parameter
   storage. `model.dtype` remains the model compute dtype.
...
| Training runs out of GPU memory           | Make sure you set `XLA_PYTHON_CLIENT_MEM_FRACTION=0.9` (or higher) before running training. The default `--sharding-strategy=full_shard` shards parameters and optimizer state across `--fsdp-devices`; `shard_grad_op` is faster only when replicated parameters fit. If full sharding still does not fit, use `--cpu-offload` (or `OPENPI_CPU_OFFLOAD=1` in `train.sh`) to keep optimizer state in pinned host memory between steps; this trades memory for host-device transfer time. You can also disable EMA to remove its extra parameter copy. |
