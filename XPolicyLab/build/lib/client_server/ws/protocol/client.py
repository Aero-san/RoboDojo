...
class PolicyEvalClientConfig:
    ...
    # Inference can temporarily occupy the policy process/GPU for longer
    # than the old 20-second keepalive window.  Keep the connection alive
    # during long vision-policy calls and let request_timeout_s handle actual
    # request failures.
...
class PolicyEvalClient:
    ...
    async def close(self) -> None:
        # Tell the server about shutdown before closing the transport.  This
        # avoids racing the server's CLOSE handler with the websocket close
        # handshake and prevents spurious keepalive/close timeout logs.
        if self._ws is not None and not self._closed:
            try:
                await self.send_close(reason="client_shutdown")
            except Exception:
                pass
        self._closed = True
    ...
    async def _recv_loop(self) -> None:
        ...
        try:
            ...
            # Normal peer shutdown; do not report it as a client error.
            close_exc = None
        except (asyncio.TimeoutError, TimeoutError):
            # The peer may have disappeared during close.  The surrounding
            # evaluation already handles transport teardown best-effort.
            close_exc = None
        except Exception as exc:
