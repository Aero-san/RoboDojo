...
class DirectRLEnv(gym.Env):
    ...
    def __del__(self):
        ...
        # ``__init__`` can fail before ``scene`` and the other runtime
        # members are created (for example when a stale SimulationContext is
        # detected).  Destructors must remain best-effort and must not mask
        # the original exception with an AttributeError.
        try:
            self.close()
        except Exception:
            pass
    ...
    def close(self):
        ...
        if not getattr(self, "_is_closed", False):
            ...
            if hasattr(self, "scene"):
                del self.scene
            if getattr(self, "viewport_camera_controller", None) is not None:
            ...
            # ``SimulationContext`` is a process-wide singleton.  It must be
            # cleared for both in-memory and USD-backed stages; otherwise a
            # subsequent eval reset/retry cannot create a new DirectRLEnv.
            if hasattr(self, "sim") and self.sim is not None:
